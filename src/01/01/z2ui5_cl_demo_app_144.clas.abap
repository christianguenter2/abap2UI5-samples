...
CLASS z2ui5_cl_demo_app_144 IMPLEMENTATION.
  ...
  METHOD set_view.
    ...
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    page->input( client->_bind_edit( val = temp2-title tab = t_tab tab_index = 1 ) ).
    ...
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    page->input( client->_bind_edit( val = temp4-value tab = t_tab tab_index = 1 ) ).
    ...
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    page->input( client->_bind_edit( val = temp6-title tab = t_tab tab_index = 2 ) ).
    ...
    IF sy-subrc <> 0.
      ASSERT 1 = 0.
    ENDIF.
    page->input( client->_bind_edit( val = temp8-value tab = t_tab tab_index = 2 ) ).
